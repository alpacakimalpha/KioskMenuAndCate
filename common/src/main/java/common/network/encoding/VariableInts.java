...
public class VariableInts {
     ...
     * 그렇게 된 상태에서, i가 7비트에 모두 들어가는 수 ({@code <= 127}) 라면,AND 연산을 하면 0이 나올 것 입니다. ( 마지막 7비트가 <code>0000000</code> 이기때문 ) <br>
     * 이 때 0이 나오면 7비트로 표현이 가능한 수고, 0이 아니라면 상위비트가 존재하는 것 입니다. ({@code 1 & 1}은 <code>1</code> 임으로. )
     * @param i 대상 정수
